"""Contract Truth v1 gate against the live ClinGen MCP registry."""
...
async def test_documentation_matches_live_mcp_contract(
    ...
    monkeypatch.setenv("CLINGEN_LINK_SNAPSHOT_PATH", str(tmp_path / "must-not-be-read.sqlite"))
    monkeypatch.chdir(tmp_path)

    from clingen_link.mcp.facade import create_clingen_mcp
    from clingen_link.services.aggregator import ClingenServices

    services = cast(ClingenServices, tool_services)
    mcp = create_clingen_mcp(service_factory=lambda: services)
