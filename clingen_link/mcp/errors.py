...
def mcp_validation_tool_error(
    *,
    ...
    ``exc`` is the underlying pydantic ValidationError when available; its structured
    ``.errors()`` drive FIXED, caller-safe ``field_errors``. When absent (a FastMCP
        ...
        async def wrapped_run(
            ...
            *,
            ...
            try:
                ...
                # FastMCP 3.x re-raises arg validation as its own (non-pydantic)
                # ValidationError whose str() embeds the caller-supplied argument
                # name/value; surfaced raw it would leak that text (and control code
                # points) to the model, and mask_error_details does NOT cover arg
                # validation. Route it through our envelope built from the structured
