...
from clingen_link.mcp.notfound_guard import (
    NotFoundGuard,
...
def create_clingen_mcp(
    ...
    # fixed name-free envelope) and fixes the on_read_resource boundary; add it FIRST
    # so it is the OUTERMOST middleware. See notfound_guard.py.
    ...
    # Layer 3: install the protocol-handler backstop LAST (after every tool/resource/
    # prompt is registered AND after the output-validation CallTool wrapper), so this
    # is the OUTERMOST wrapper on the raw CallTool/ReadResource/GetPrompt handlers --
    # it catches the unknown-tool *return* path and any resource/prompt dispatch error
    # that would echo the requested name/URI (the only layer covering unknown-prompt).
