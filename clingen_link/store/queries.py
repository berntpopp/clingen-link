...
def search_genes(conn: sqlite3.Connection, query: str, *, limit: int = 25) -> list[dict[str, Any]]:
    """Return gene index rows whose symbol or alias prefix-matches ``query``.

    Resolution to a single canonical symbol is the store's job; this returns the
    candidate set for the ``search_genes`` tool's listing.
    """
    like = f"{query}%"
    rows = conn.execute(
        "SELECT g.symbol, g.hgnc_id, g.name, g.has_validity, g.has_dosage, "
        "g.has_actionability, g.erepo_variant_count FROM gene g "
        "WHERE g.symbol LIKE ? COLLATE NOCASE "
        "UNION "
        "SELECT g.symbol, g.hgnc_id, g.name, g.has_validity, g.has_dosage, "
        "g.has_actionability, g.erepo_variant_count FROM gene g "
        "JOIN gene_alias a ON a.symbol = g.symbol "
